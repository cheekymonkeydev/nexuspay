...
var BASE_URL = (process.env.NEXUSPAY_URL ?? "").replace(/\/$/, "");
var API_KEY = process.env.NEXUSPAY_API_KEY ?? "";
if (!BASE_URL) {
...
async function handleTool(name, args) {
  switch (name) {
    case "nexuspay_get_balance": {
      const wallet = await api(`/wallets/${args.agentId}`);
      return text({
    ...
    case "nexuspay_list_wallets": {
      ...
      return text({
        ...
        totalBalance: Array.isArray(list) ? list.reduce((s, w) => s + w.balanceUsdc, 0) : 0
      });
    }
    case "nexuspay_create_wallet": {
      const wallet = await api("/wallets", "POST", { agentId: args.agentId });
      return text({
        success: true,
        agentId: wallet.agentId,
        address: wallet.address,
