import type { MppChallenge, MppChallengeData, MppAdapterConfig } from "./types.js";
import { b64url, fromB64url, computeChallengeId } from "./crypto.js";

const MPP_METHOD  = "nexuspay" as const;
const MPP_INTENT  = "charge"   as const;
const DEFAULT_TTL = 5 * 60 * 1000; // 5 minutes

/* ─── Realm resolution ───────────────────────────────────────────────────── */

function getRealm(config: MppAdapterConfig): string {
  if (config.realm) return config.realm;
  if (process.env.NEXUSPAY_MPP_REALM) return process.env.NEXUSPAY_MPP_REALM;
  const appUrl = process.env.NEXT_PUBLIC_APP_URL;
  if (appUrl) {
    try { return new URL(appUrl).hostname; } catch { /* fall through */ }
  }
  return "localhost";
}
...
export function buildChallengeResponse(challenge: MppChallenge, price: number): Response {
  const data: MppChallengeData = JSON.parse(fromB64url(challenge.request));
  return new Response(
    JSON.stringify({
