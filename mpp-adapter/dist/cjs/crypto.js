...
exports.verifyChallengeId = verifyChallengeId;
 ...
 */
function computeChallengeId(realm, method, intent, request, expires) {
    const input = `${realm}|${method}|${intent}|${request}|${expires}`;
    return (0, crypto_1.createHmac)("sha256", getSecret()).update(input).digest("base64url");
}
/**
 * Constant-time HMAC verification to prevent timing attacks.
 */
function verifyChallengeId(id, realm, method, intent, request, expires) {
    try {
