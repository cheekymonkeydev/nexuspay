...
/* ─── Challenge creation ─────────────────────────────────────────────────── */
function createChallenge(endpointPath, config) {
    ...
    const request = (0, crypto_js_1.b64url)(JSON.stringify(data));
    const id = (0, crypto_js_1.computeChallengeId)(realm, MPP_METHOD, MPP_INTENT, request, expires);
    return { id, realm, method: MPP_METHOD, intent: MPP_INTENT, request };
}
/* ─── Header builders ────────────────────────────────────────────────────── */
function buildWwwAuthenticate(challenge) {
...
/* ─── 402 Response ───────────────────────────────────────────────────────── */
function buildChallengeResponse(challenge, price) {
    const data = JSON.parse((0, crypto_js_1.fromB64url)(challenge.request));
