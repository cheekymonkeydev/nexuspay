...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const awaitingApproval = req.nextUrl.searchParams.get("awaitingApproval");
    ...
    // Filter for transactions held pending human approval
    if (awaitingApproval === "true") {
      where.status = "PENDING";
      where.policyChecks = { path: ["requiresApproval"], equals: true };
    }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!policyResult.passed) {
      // ── Approval gate: hold funds as PENDING, do NOT reject ───────────────
      if (policyResult.requiresApproval) {
        ...
        if (decremented.count === 0) return err("Insufficient balance (concurrent transaction)", 409);
        ...
        const held = await prisma.transaction.create({
          data: {
            ...
            memo:         input.memo,
            status:       "PENDING",
            policyChecks: { ...policyResult.checks, requiresApproval: true },
            failureReason: "Awaiting manual approval",
          },
        });
        return ok({ ...held, awaitingApproval: true });
      }
    ...
    try {
      ...
      // Settlement failed — refund balance, mark FAILED
